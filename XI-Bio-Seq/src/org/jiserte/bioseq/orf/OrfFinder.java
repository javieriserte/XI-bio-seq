...
import org.jiserte.bioseq.orf.analysis.ShowPositions;
import org.jiserte.bioseq.orf.datastructures.OrfComposer;
import org.jiserte.bioseq.orf.datastructures.OrfMarks;
import org.jiserte.bioseq.orf.replication.Replicate;
import org.jiserte.bioseq.orf.replication.ReplicateCircular;
import org.jiserte.bioseq.orf.replication.ReplicateLinear;
import org.jiserte.bioseq.orf.replication.Replicon;
import org.jiserte.bioseq.orf.seqsource.FastaSequenceSource;
import org.jiserte.bioseq.orf.seqsource.SequenceSource;
import org.jiserte.bioseq.orf.seqsource.TextSequenceSource;
...
public class OrfFinder {
  ...
  public static void main(String[] args) {
    ...
    // STEP TWO:
    // DEFINE THE POSSIBLE OPTIONS ACCEPTED IN THE COMMAND LINE. (TAKE CARE OF
    // AVOID AMBIGUITY)
    SingleOption input = new SingleOption(parser, System.in, "-in",
        InputStreamParameter.getParameter());
    NoOption cir = new NoOption(parser, "-circular");
    SingleOption minsize = new SingleOption(parser, 100, "-min",
        IntegerParameter.getParameter());
    SingleOption outfile = new SingleOption(parser, System.out, "-outfile",
  ...
  private static OrfAnalysis getAnalysis(SingleOption minsize,
    ...
    if (marksOpt.isPresent())
      a = new ShowMarks(replicate, (Integer) minsize.getValue(), frames);
    else if (largestOpt.isPresent())
  ...
  private static Integer[] getFrames(MultipleOption frameOpt) {
    ...
    if (frameOpt.getValues().length == 1
        && (Integer) frameOpt.getValues()[0] == 0) {
  ...
  protected static void printHelp(PrintStream out) {
    out.println("OrfFinder ver 0.0.2");
    out.println("");
    out.println("   Options:");
   ...
   * @param frame
   *          <code>0</code> for All, <code>1-3</code> for specific frame.
   * @param minSize
   *          minimum size of the ORF to report, the size is measured in bases,
   *          no amino acids.
   * @param isCircular
   *          True if the sequence is from a circular DNA molecule (i.e. a
   *          Plasmid).
   * 
  ...
  public static List<String> allOrfsInThisStrand(String sequence, int minSize,
    ...
    return retriveORFs(composer.getSequence(), composer.getUnitlength(),
        composer.getMarks(), minSize);

  }

  public static OrfComposer getOrfComposer(String sequence,
      Replicate replicator, Integer[] frames) {
    ...
    Replicon rep = replicator.attempToReplicateSequence(sequence, unitLength,
        ATGs, STOPs);

    OrfMarks marks = separateByFrame(frames, rep.getATG(), rep.getStop());

    marks = excludeAdjacentATGorSTOP(marks, replicator.isCircular(), unitLength);
  ...
  /**
   * Retrieve the ORFs from a sequence, given a list of ATGs and STOPs mark
   * position. There is expected that there are not two ATGs or two STOPs
   * adjacent in the list. Usually the method
   * <code>excludeAdjacentATGorSTOP</code> is called before this.
   * 
   * @param sequence
   *          the sequence from which the ORFs will be extracted.
   * @param unitLength
   *          is the Sequence minimum unit length.
   * @param ATGsAndSTOPsByFrame
   *          an <code>array</code> containing the ATGs and STOPs mark
   *          positions.
   * @param ATGorStop
   *          is a list of boolean values that is a reference of which elements
   *          in ATGsAnsSTOPsByFrame list are ATG o STOP. When ATGorSTOP[index]
   *          is true, then ATGsAnsSTOPsByFrame[index] is the position of an
   *          ATG. When ATGorSTOP[index] is false, then
   *          ATGsAnsSTOPsByFrame[index] is the position of a STOP.
   * @param result
  ...
  protected static List<String> retriveORFs(String sequence, int unitLength,
    ...
    for (int cframe = 0; cframe < 3; cframe++) {
      ...
      for (int i = 0; i < ATGsAndSTOPsByFrame[cframe].size(); i++) {
        ...
        if (ATGorStop[cframe].get(i)
            && (i + 1) < ATGsAndSTOPsByFrame[cframe].size()) {
          ...
          if (atgpos < unitLength && (endIndex - atgpos) >= minSize) {
            ...
            // checks that the length of the orf is greater or equal than
            // minsize
  ...
  /**
   * Detect, in a list of ATG and STOP, two adjacent ATG or two adjacent STOP
   * and keeps the lower one, removing the other.
   * 
   * @param ATGsAnsSTOPsByFrame
   *          is a list of ATG and STOP mark positions
   * @param ATGorStop
   *          is a list of boolean values that is a reference of which elements
   *          in ATGsAnsSTOPsByFrame list are ATG o STOP. When ATGorSTOP[index]
   *          is true, then ATGsAnsSTOPsByFrame[index] is the position of an
  ...
  protected static OrfMarks excludeAdjacentATGorSTOP(OrfMarks marks,
      boolean isCircular, int seqUnitLength) {

    // //////////////////////////////////////////////////////////////////////////
    // retrieves all ATG and Stop positions for all frames
    List<Integer>[] ATGsAndSTOPsByFrame = marks.getATGsAndSTOPsByFrame();
    List<Boolean>[] ATGorStop = marks.getATGorStop();
    // //////////////////////////////////////////////////////////////////////////

    // //////////////////////////////////////////////////////////////////////////
    // Remove adjacents ATG and STOPs
    for (int cframe = 0; cframe < 3; cframe++) {

      // ////////////////////////////////////////////////////////////////////////
      // First remove all trivial adjacent ATGs and STOPs
      // When two consecutive ATGs or Stops appear, removes the second one.
      // The iteration makes that always keep the first, when three of more
      // consecutive ATGs or STOPs appears.
      if (ATGsAndSTOPsByFrame[cframe].size() > 1) {
        int lastIndex = 0;
        boolean lastWasATG = ATGorStop[cframe].get(lastIndex);
        for (int i = 1; i < ATGsAndSTOPsByFrame[cframe].size(); i++) {
          if (ATGorStop[cframe].get(i) == lastWasATG) {
            ATGsAndSTOPsByFrame[cframe].remove(i);
    ...
    // End of iterate over frames
    // /////////////////////////////////////////////////////////////////////////

    // /////////////////////////////////////////////////////////////////////////
    // Remove adjacent ATG and STOPs
    for (int cframe = 0; cframe < 3; cframe++) {
      // If circular option is used, and the first and the last mark of
      // a frame is an ATG, then the first one must be removed
      if (isCircular && !ATGorStop[cframe].isEmpty()) {
        boolean hasLastProperATG = false;
        int indexOfLastProperATG = 0;
        int index = 0;
        while (ATGsAndSTOPsByFrame[cframe].size() > index
            && ATGsAndSTOPsByFrame[cframe].get(index) < seqUnitLength) {
          hasLastProperATG = ATGorStop[cframe].get(index);
          if (hasLastProperATG) {
            indexOfLastProperATG = ATGsAndSTOPsByFrame[cframe].get(index);
          }
          index++;
        }
        int nframe = 2 - (seqUnitLength - indexOfLastProperATG - 1) % 3;

        if (ATGorStop[nframe].get(0) && hasLastProperATG) {
          ATGsAndSTOPsByFrame[nframe].remove(0);
          ATGorStop[nframe].remove(0);
        }
      }
    }
    // End of removing initial ATGs with circular option
    // /////////////////////////////////////////////////////////////////////////
    return marks;
  }
   ...
   * 
   * @param frame
   *          is the reading frame of the ATGs and STOPs positions to be
   *          retrieved. frame can be 0,1,2 or 3. 1 to 3 for each reading frame
   *          and 0 for all.
   * @param ATGs
   *          an array of ATG marks positions.
   * @param STOPs
   *          an array of STOP marks positions.
   * 
   * @param ATGsAnsSTOPsByFrame
   * @param ATGorStop
   */
  protected static OrfMarks separateByFrame(Integer[] frames, int[] ATGs,
      int[] STOPs) {
    ...
    for (int frame : frames) {
      ...
      if ((cframe + 1) == frame || frame == 0) {
        ...
        int StopIndex = 0;
        int currentATG = 0;
        if (atgIndex < ATGs.length)
          currentATG = ATGs[atgIndex];
        int currentSTOP = 0;
        if (StopIndex < STOPs.length)
          currentSTOP = STOPs[StopIndex];
        while (atgIndex < ATGs.length || StopIndex < STOPs.length) {

          // while(atgIndex<ATGs.length &&
          // (currentATG%3<0?(currentATG%3+3):currentATG%3) != cframe) {
          ...
          while (StopIndex < STOPs.length && currentSTOP % 3 != cframe) {
            // tries to get the next stop in the current frame
            StopIndex++;
            if (StopIndex < STOPs.length)
              currentSTOP = STOPs[StopIndex];
          }
          ...
          if (atgIndex == ATGs.length && StopIndex == STOPs.length) {
            ...
            continue;
          } else if (StopIndex == STOPs.length && atgIndex < ATGs.length) {
            ...
            if (atgIndex < ATGs.length)
              currentATG = ATGs[atgIndex];
          } else if (atgIndex == ATGs.length && StopIndex < STOPs.length) {
            // Stop reach the end and ATGs do not.
